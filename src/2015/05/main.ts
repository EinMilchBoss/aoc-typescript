...
export function solveFirst(input: string): number {
    ...
    input.split('\n').forEach(line => {
        if (countVowels(line) >= 3 && hasLetterAdjacent(line) && !includesSome(line, naughtyParts)) {
            ++niceStringCount;
        }
    });
...
function main() {
    const input = fs.readFileSync(util.getResource('2015', '05', 'input.txt'), 'utf8').toString();
